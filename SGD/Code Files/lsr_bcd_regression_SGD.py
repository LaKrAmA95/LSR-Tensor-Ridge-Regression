...
import LSR_Tensor_2D_v1_SGD
import numpy as np
from optimization_SGD import objective_function_tensor
...
def lsr_bcd_regression(lsr_ten, training_data: np.ndarray, training_labels: np.ndarray, hypers: dict, intercept = False):
    ...
    factor_matrix_models = [[SGDRegressor(alpha = lambda1, fit_intercept = intercept,learning_rate='optimal',max_iter=10000,) for k in range(len(ranks))] for s in range(sep_rank)]
    core_tensor_model = SGDRegressor(alpha = lambda1, fit_intercept = intercept)
