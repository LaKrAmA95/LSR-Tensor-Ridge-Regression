...
def lsr_bcd_regression(lsr_ten, training_data: np.ndarray, training_labels: np.ndarray, hypers: dict, intercept = False):
    ...
    factor_matrix_models = [[SGDRegressor(alpha = lambda1, fit_intercept = intercept,learning_rate='adaptive',max_iter=10000,) for k in range(len(ranks))] for s in range(sep_rank)]
