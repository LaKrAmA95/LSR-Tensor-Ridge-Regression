...
def lsr_bcd_regression(lsr_ten, training_data: np.ndarray, training_labels: np.ndarray, hypers: dict, intercept = False):
    ...
    factor_matrix_models = [[Ridge(alpha = lambda1[0], solver = 'svd', fit_intercept = intercept) for k in range(len(ranks))] for s in range(sep_rank)]
    core_tensor_model = Ridge(alpha = lambda1[1], solver = 'svd', fit_intercept = intercept)
    ...
    for iteration in range(max_iter):
        ...
        for s in range(sep_rank):
            for k in range(len(ranks)):
                ...
                gradient_value = (-2 * Omega.T) @ (y_tilde.reshape(-1,1) - Omega @ bk  - z) + (2 * lambda1[0] * bk)
        ...
        Omega = X_tilde
        z = b if intercept else 0
        gradient_value = (-2 * Omega.T) @ (y_tilde.reshape(-1,1) - Omega @ g  - z) + (2 * lambda1[1] * g)
