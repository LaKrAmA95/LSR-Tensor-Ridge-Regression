...
def objective_function_tensor_sep(y: np.ndarray, X: np.ndarray, B: np.ndarray,lsr_ten, alpha,b = None):
    ...
    for s in range(separation):
       for k in range(tucker):
          regularizer += alpha[0]*(np.linalg.norm(lsr_ten.factor_matrices[s][k])**2)
    regularizer = regularizer + alpha[1]*(np.linalg.norm(lsr_ten.core_tensor)**2)

    if b is not None:
      b = b.flatten()
      function = (np.linalg.norm(y - I -b) ** 2) + (regularizer)
    else:
      function = (np.linalg.norm(y - I) ** 2) + (regularizer)
    return function
