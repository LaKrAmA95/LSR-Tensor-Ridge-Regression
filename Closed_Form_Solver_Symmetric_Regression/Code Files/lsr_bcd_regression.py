...
def lsr_bcd_regression(lsr_ten, training_data: np.ndarray, training_labels: np.ndarray, hypers: dict, intercept = False):
    ...
    for iteration in range(max_iter):
        ...
        for s in range(sep_rank):
            ...
            for k in range(len(ranks)):
                ...
                #holding the factor matrices
                hold_factor_matrices.append(Bk1)
        ...
        #constraint for symmetricity
        sum_factor_matrices = np.sum(hold_factor_matrices,axis =0)
        average_factor_matrix = sum_factor_matrices / len(hold_factor_matrices)
        Bk1_same = average_factor_matrix
               
        #Update Residuals and store updated factor matrix
        factor_residuals[s][:] = np.linalg.norm(Bk1_same - Bk)
        
        #updating the lsr_tensor object 
        for k in range(len(ranks)):
            updated_factor_matrices[s, k] = Bk1_same
            lsr_ten.update_factor_matrix(s, k, updated_factor_matrices[s, k])

        # Starting to solve the core tensor problem 
        ...
        #projecting the core tensor into the space of symmetric matrices
        G_sym = (Gk1 + Gk1.T ) / 2 
        
        #Update Residuals and store updated Core Tensor
        core_residual = np.linalg.norm(G_sym - Gk)
        updated_core_tensor = G_sym
    ...
    return lsr_ten, objective_function_values, gradient_values,factor_core_iteration